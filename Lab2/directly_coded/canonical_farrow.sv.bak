...
// use this as starter code for Farrow project
// ECE260C
...
module canonical_farrow(				    // baseline design, to be optimized by students 
  input                     clk,
                            reset,
  input        signed[15:0] mu,				// format 2 integer + 14 fraction
  input        signed[15:0] dat_in,			// input formatted as desired by user -- be consistent
...
// last row ...
  ...
  always_comb begin 					   // what's inefficient with this path? 
	p[0] = muq*s[0];
	p[1] = muq*(s[1]+(p[0]>>>14));		   // normalization of fractional coefs.
	p[2] = muq*(s[2]+(p[1]>>>14)); 
	sum  =      s[3]+(p[2]>>>14); 
  end
...
// input, internal pipe, output registers
  always @(posedge clk) 
    if(reset) begin
      ...
      muq     <= mu;					   // register frequency offset input
      dff[0]  <= dat_in;				   // internal data pipeline
      dff[1]  <= dff[0];				   // supports sum(f(x(k),x(k-1),x(k-2),x(k-3)
	  dff[2]  <= dff[1];
	  dff[3]  <= dff[2];
	  dat_out <= sum;	                   // register final filter result output
    end
